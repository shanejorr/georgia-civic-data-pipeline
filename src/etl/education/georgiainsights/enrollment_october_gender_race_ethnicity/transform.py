"""Transform bronze enrollment_october_gender_race_ethnicity files into gold facts.

Source: Georgia Insights (GaDOE) "FTE Enrollment by Race/Ethnicity and
Gender" — the October Cycle 1 (``-1``) FTE headcount of Georgia public school
...
fiscal years 2010-2026. The companion topic
``enrollment_march_gender_race_ethnicity`` reports the same breakdown from
the March Cycle 3 count; the race/gender vocabulary both topics share lives
in ``src/etl/education/georgiainsights/_enrollment_race_lookups.py``.

Design decisions (every invariant re-verified against THIS topic's 34 bronze
CSVs during authoring — see bronze-data-structure.md):

- **Single era, two files per year.** District and School counts live in
  separate CSVs per year; all 34 files share one post-strip column set
  ...
  (notably ``Gender`` has 1 leading space in District files but 2 in School
  files). Each CSV opens with a 4-line preamble; the reader skips it,
  verifies the line-2 title, and cross-checks the preamble's
  ``Fiscal Year YYYY-1`` against the filename year (all 34 agree). Year
  exists ONLY there — there is no year column in the data.
...
- **Read loss is measured, not assumed.** The local CSV reader counts the
  ...
  file. Verified raw == parsed for all 34 files.
...
- **Split Asian / Pacific Islander convention (section 5b)**: bronze has
  ...
  over race) — the seven buckets partition total enrollment (October 2026
  state Total: 1,379,097 across the six race buckets + 335,934 Hispanic =
  1,715,031, matching Georgia's published FTE October 2025 enrollment).
...
- **Gender ``Total`` rows are dropped** before unpivot: ``Total`` equals
  ``Female + Male`` on every fully published cell (verified on all 123,666
  complete (geography, race) triples across 34 files — 0 violations), so
...
- **State rows are published twice per year** — both the District and the
  ...
  all 17 years). The natural-key collision guard proves the twins agree,
...
- **Suppression**: ``*`` is the only marker in any of the seven count
  columns across all 34 files (verified); it becomes NULL via the reader's
...
BRONZE_DIR = Path(
    "data/bronze/education/georgiainsights/enrollment_october_gender_race_ethnicity"
)
GOLD_DIR = Path("data/gold/education/enrollment_october_gender_race_ethnicity")
SOURCE_URL = "https://georgiainsights.gadoe.org/data-downloads/"

# Filename: "... Fiscal Year2010-1 District.csv" (no space before the year;
# `-1` = Cycle 1 / October). The preamble's line 2 writes the year WITH a
# space ("Fiscal Year 2010-1 Data Report") — two distinct patterns.
FILENAME_PATTERN = re.compile(r"Fiscal Year(\d{4})-1 (District|School)\.csv$")
...
def _read_bronze_csv(path: Path, year: int) -> tuple[pl.DataFrame, dict]:
    ...
    suppression marker, verified across all 34 files) as a null value.
...
def _transform_file_frame(
    ...
    Single-era topic: all 34 files share one post-strip column set; the only
    ...
    # across all 17 years; guarded below).
    ...
    df = df.with_columns(
        ...
        # any stray non-numeric value (none observed across 34 files).
        *[
...
def transform_file(path: Path, manifest: TransformManifest) -> pl.DataFrame | None:
    ...
    manifest.record_file(
        path, year, "era_1_2010_2026_fte_cycle1", df.height, df.columns
    )
...
def main() -> None:
    ...
    # the 3 state rows — verified identical for all 17 years). The guard
...
def _emit_contract_and_readme(year_range: tuple[int, int]) -> None:
    ...
    write_data_dictionary(
        ...
        description=(
            ...
            "Gender — the October Cycle 1 FTE headcount of Georgia public "
            ...
            "years 2010-2026. This is the fall snapshot; the companion "
            "topic enrollment_march_gender_race_ethnicity reports the "
            "March Cycle 3 count. Source gender `Total` rows are dropped "
            "(redundant with Female + Male)."
        ),
        title="October Enrollment by Race and Gender",
        summary=(
            "Public school student headcounts by race/ethnicity and gender "
            "from the October FTE count, per school, district, and state, "
            "2010-2026."
        ),
        columns=[
            {
            ...
            },
            {
                ...
                "description": (
                    ...
                    "(the observed suppression floor across all 34 bronze "
        ...
        notes=[
            (
                ...
                "are non-Hispanic. Verified for October 2026 state totals: "
                "1,379,097 (six race buckets) + 335,934 (Hispanic) = "
                "1,715,031 = published statewide FTE enrollment."
            ),
            (
                "Source `Total` gender rows are dropped (Total = Female + "
                "Male on all 123,666 fully published cells across 34 bronze "
            ...
            (
                ...
                "suppressed at the school level (~99.9%% in 2026); the "
            ...
            (
                ...
                "(verified equal for all 17 years); the transform keeps "
        ...
        quality_checks=[
            {
                ...
                "description": (
                    ...
                    "the minimum non-null value across all 34 bronze files "
                    "2010-2026 is exactly 15; smaller cells are suppressed "
                    "to `*`)."
                ),
                "dimension": "accuracy",
                "query": (
