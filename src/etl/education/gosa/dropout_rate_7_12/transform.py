"""Transform bronze dropout_rate_7_12 files into gold fact tables.

Source: Governor's Office of Student Achievement (GOSA) — 7-12 Dropouts,
...
grade 7-12 dropouts (``num_dropouts``) and the dropout rate
...
- **Two near-identical eras, one pipeline.** Era 1 (2023-2024) adds a leading
  constant ``#RPT_NAME`` column ("7-12 Dropouts"); Era 2 (2011-2022) is the
...
- **Heavy suppression is bronze-real.** GOSA suppresses any cell below the
  n=10 reporting threshold, which blanks ~78-83%% of school-level rows in
...
- **Demographic labels carry a constant prefix.** Every ``LABEL_LVL_1_DESC``
  starts with ``"7-12 Drop Outs -"``; the prefix is stripped and the
...
- **Limited English Proficient gap.** The 2020-2022 files drop the Limited
  ...
  The transform maps whatever labels appear; the gap is pinned as a quality
  check so a silent reappearance/disappearance is caught.
- **2022 mislabeled district aggregates, reclassified.** The 2022 file
  ...
  row — 2022 has none for these codes).
- **No §4b masks.** A full scan of all 14 bronze files found no impossible
  values: ``PROGRAM_TOTAL`` spans [10, 24061] (non-negative integers) and
...
BRONZE_DIR = Path("data/bronze/education/gosa/dropout_rate_7_12")
GOLD_DIR = Path("data/gold/education/dropout_rate_7_12")
...
DEMOGRAPHIC_LABEL_PREFIX = "7-12 Drop Outs -"
...
def main() -> None:
    """Run the full bronze-to-gold pipeline for dropout_rate_7_12."""
    ...
    # Pre-export sanity. Suppression NULL rates are heavy (~78-83% of school
    # rows) but uniform across years, so no spike is expected.
...
def _emit_contract_and_readme(year_range: tuple[int, int]) -> None:
    ...
    write_data_dictionary(
        ...
        name=TOPIC,
        description=(
            "Number and rate of grade 7-12 dropouts for Georgia public "
        ...
        title="Grade 7-12 Dropout Rates",
        summary=(
            "Number and rate of grade 7-12 students who dropped out, by "
            "Georgia school, district, and demographic subgroup, 2011-2024."
        ),
        columns=[
            {
                "name": "year",
            ...
            },
            {
                ...
                "description": (
                    ...
                    ">= 10 because GOSA suppresses smaller cells; ~78-83%% "
            ...
            },
            {
                ...
                "example": 0.023,
                "short_description": (
                    "Share of the subgroup's grade 7-12 students who dropped "
                    "out during the school year, on a 0-1 scale; lower is "
                    "better."
                ),
                "null_meaning": (
                    "Suppressed by GOSA (cell below the n=10 reporting "
                ...
                "description": (
                    "Grade 7-12 dropout rate as a proportion (0-1 scale): "
                    "dropouts divided by the subgroup's grade 7-12 "
        ...
        limitations=(
            ...
            "below the n=10 reporting threshold, which blanks ~78-83%% of "
        ...
        notes=[
            ...
            (
                "Middle schools appear in this report: grade 7-8 dropouts "
                "occur at schools serving only grades 5-8 or 6-8. The "
                "source's GRADES_SERVED_DESC (grade span served) is "
                "institution metadata, not carried into the fact table; "
                "district and school names live in the dimension tables."
            ),
            (
                "Metric NULL rates are high (~78-83%% of school rows) but "
